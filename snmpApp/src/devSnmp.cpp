  ...
  if (op == NETSNMP_CALLBACK_OP_RECEIVED_MESSAGE) {
    ...
    if (errCode == SNMP_ERR_NOERROR) {
      ...
      if (errCode == SNMP_ERR_NOSUCHNAME) {
        ...
        // NABO, January 2017:
        // Both snmpget and snmp_send occaisonally cannot find variables in 
        // AKCP's SPAGENT-MIB.  Successive tries succeed for snmpget, but
        // flagged_read_bad stops future tries.  We could count consecutive
        // failures and then set this flag if past some threshold, or just
        // don't set it at all, or try to debug what's going on in that MIB.
        
        //flagged_read_bad = errCode;
        
        printf("***NAB devSnmp %s (GET) flagged bad, error = %ld (%s)\n",
                oidBase.oidStr, errCode, snmp_errstring(errCode));
      }
      retVal = epicsError;
    }
